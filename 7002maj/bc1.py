...
print ("The F1_Score of Bagging Classifier on testing data is: " + str(f1))
testSet = [[1,1,2,1,3,1,1,2,1,1,2,3]]
test = pd.DataFrame(testSet)
predictions = classifier.predict(test)
print('Bagging Classifier Prediction on the first test set is:',predictions)
testSet = [[2,1,3,2,2,2,1,3,2,1,2,2]]
test = pd.DataFrame(testSet)
predictions = classifier.predict(test)
print('Bagging Classifier Prediction on the second test set is:',predictions)
testSet = [[3,1,2,3,3,3,1,3,2,3,3,1]]
test = pd.DataFrame(testSet)
predictions = classifier.predict(test)
print('Bagging Classifier Prediction on the third test set is:',predictions)
